...
const YearlyPlans = ({ loggedInUser, setSelectedPlan }) => {
    ...
    const getTableData = () => {
        axios.get('/getTableData').then((res) => {
            setFeatureTable(res.data.tableData.splice(7, 1).concat(res.data.tableData.splice(1, 2)));
            setDevicesTable(res.data.tableData.splice(1, 4));
        })
    }
